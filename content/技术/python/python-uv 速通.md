...
可用于替代 `pip` + `venv` + `pyenv` + `poetry` 等多工具的复杂组合
...
**1. 添加依赖**：`uv add <指定包1> <指定包2>`（`--dev <开发环境>` 添加开发依赖）
1. 将依赖信息写入 `pyproject.toml`。
2. 解析并锁定所有依赖的精确版本，生成 `uv.lock` 文件。
3. 在项目专属的虚拟环境 `.venv` 中安装这个包。

**2. 移除依赖**： `uv remove <指定包>`

**3. 同步项目环境**：`uv sync`
- 此命令会读取 `uv.lock` 文件，在新环境中创建相同的依赖包版本

**4. 更新锁文件**：`uv lock`
- 此命令会创建或更新 `uv.lock` 文件

依赖锁定文件：**uv.lock**
- 项目**所有直接依赖**（你主动添加的包，如 `requests`）的精确版本号。
...
| 项目阶段     | 核心任务            | `uv` 命令                          |
| -------- | --------------- | -------------------------------- |
| **项目诞生** | 创建项目、指定Python版本 | `uv init`                        |
| **项目成长** | 添加/移除依赖、同步环境    | `uv add`, `uv remove`, `uv sync` |
| **项目运行** | 执行脚本、运行临时工具     | `uv run`, `uvx`                  |
| **项目交付** | 构建包、发布到PyPI     | `uv build`, `uv publish`         |

## 参考
1.  [uv - Astral 官方文档](https://docs.astral.org.cn/uv/)
2. [功能指令列表 | uv - Astral 文档](https://docs.astral.org.cn/uv/getting-started/features/#scripts)
3. [用 uv 管理 Python 项目](https://realpython.com/python-uv/)