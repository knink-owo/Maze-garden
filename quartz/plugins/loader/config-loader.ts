...
import MazeIcons from "../../components/maze/MazeIcons"
import Backlinks from "../../components/maze/Backlinks"
...
export async function loadQuartzConfig(
  ...
  // 迷宫花园 · 自定义组件注入（组件源码见 quartz/components/maze/）
  const withMazeIcons = (pageTypeLayout: Partial<FullPageLayout>): Partial<FullPageLayout> => ({
    ...pageTypeLayout,
    header: [...(pageTypeLayout.header ?? layout.defaults.header ?? []), MazeIcons()],
  })
  ...
  if (layout.byPageType.content) {
    layout.byPageType.content = withMazeAfterBody(withMazeIcons(layout.byPageType.content))
  }
  if (layout.byPageType.folder) {
    layout.byPageType.folder = withMazeIcons(layout.byPageType.folder)
  }
  if (layout.byPageType.tag) {
    layout.byPageType.tag = withMazeIcons(layout.byPageType.tag)
  }
